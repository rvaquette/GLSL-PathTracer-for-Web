...
export default class Mesh {
     ...
     */
    static loadObjFile(objectData: string, options?: MeshOptions) : Mesh[] {
        options = options || {};
    ...
    */
        let mesh: Mesh = new Mesh(options);
        
        let verts = [];
        let vertNormals = [];
        let textures = [];
        let materialNamesByIndex = [];
        let materialIndicesByName: MaterialNameToIndex = {};
        // keep track of what material we've seen last
        let currentMaterialIndex = -1;
        let currentObjectByMaterialIndex = 0;
        // unpacking stuff
        let unpacked: UnpackedAttrs = {
            verts: [],
            norms: [],
            textures: [],
            hashindices: {},
        ...
        for (let line of lines) {
            ...
            if (OBJECT_RE.test(line)) {
                if (mesh) {
                    mesh.finalizeMesh(options, unpacked, currentObjectByMaterialIndex, materialNamesByIndex, materialIndicesByName);
    ...
    finalizeMesh(options: MeshOptions, unpacked: UnpackedAttrs, currentObjectByMaterialIndex: number, materialNamesByIndex: string[], materialIndicesByName: MaterialNameToIndex) {
