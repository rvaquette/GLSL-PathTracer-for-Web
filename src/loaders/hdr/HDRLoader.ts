...
function RGBE_ReadHeader(uint8array: Uint8Array): HDRInfo {
    let height: number = 0;
    let width: number = 0;
 ...
 */
// eslint-disable-next-line @typescript-eslint/naming-convention
function RGBE_ReadPixels(uint8array: Uint8Array, hdrInfo: HDRInfo): Float32Array {
...
export class HDRLoader {
     ...
     * @returns A promise that resolves to a Float32Array containing the texture data.
     */
    static async loadHDRAsync(filename: string): Promise<[Float32Array, number, number]> {
        const response = await loadFile(filename);
        const arrayBuffer = await response.arrayBuffer();
        const uint8array = new Uint8Array(arrayBuffer);

        const hdrInfo = RGBE_ReadHeader(uint8array);
        return [ RGBE_ReadPixels(uint8array, hdrInfo), hdrInfo.width, hdrInfo.height ];
    }
}