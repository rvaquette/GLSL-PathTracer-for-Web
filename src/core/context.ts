...
export class Context {
    static _instance: Context | null = null;

    private document: Document;
    private canvas: HTMLCanvasElement;
    private gl: GL;

    private constructor(document: Document, canvas: HTMLCanvasElement) {
        ...
        if (GL.profiling) this.gl.getExtension('WEBGL_debug_shaders');
        ...
        this.canvas.addEventListener("keydown", async (event) => {
            ImGui.keydown(event as KeyboardEvent);
            event.preventDefault();
        }, false);
        this.canvas.addEventListener("keyup", async (event) => {
            ImGui.keyup(event as KeyboardEvent);
            event.preventDefault();
        }, false);
    ...
    static setInstance(document: Document, canvas: HTMLCanvasElement): void {
