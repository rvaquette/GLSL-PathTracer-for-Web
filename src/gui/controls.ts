...
*/
        ...
        if (scene.meshInstances.length === 0) 
            return;
        ...
        for (let i = 0; i < scene.meshInstances.length; i++) {
            listboxItems.push(scene.meshInstances[i].name);
        }

        let instances = objects.add({ instance: scene.meshInstances[0].name }, 'instance', listboxItems);

        let materials = objects.addFolder('Materials').close();
 
        materials = Controls.onInstanceChanged(objects, materials, scene.meshInstances[0].name, main);
    ...
    static onInstanceChanged(objects: GUI, materials: GUI, value: string, main: Main): GUI {
        ...
        let selectedInstance = scene.meshInstances.findIndex((instance) => instance.name === value);

        // Material Properties
        const mat = scene.materials[scene.meshInstances[selectedInstance].materialID];
        ...
        materials.addColor({ rgb: { r: albedo.x, g: albedo.y, b: albedo.z} }, 'rgb').listen().name('Uniform Light Color (Gamma Corrected)').onChange((value: { r: number, g: number, b: number}) => {
        ...
        let mediumTypeGui = materials.addFolder('Medium Type').close();
        ...
        materials.add({ alphaMode: alphaModeAsString }, 'alphaMode', ['Opaque', 'Blend', 'Mask']).listen().name('Alpha Mode').onChange((value: string) => {
            ...
            if (alphaMode != AlphaMode.Opaque)
                ...
                opacity = materials.add(mat, 'opacity', 0.0, 1.0).listen().name('Opacity').onChange((value: any) => {
                    main.objectPropChanged = true;
                });
        });
        ...
        // // Transforms
        // let transforms = materials.addFolder('Transforms').close();

        // const meshInstance = scene.meshInstances[main.selectedInstance];
        // const xform = meshInstance.transform;

        // let matrixTranslation: Vec3, matrixRotation: Vec3, matrixScale: Vec3;
        // ({ translation: matrixTranslation, rotation: matrixRotation, scale: matrixScale } = xform.decompose());
        
        // transforms.add({ x: matrixTranslation.x }, 'x').listen().name('TrX').onChange((value: number) => {
        //     Controls.onTransformChanged(matrixTranslation, matrixScale, matrixRotation, meshInstance);
        // });
        // transforms.add({ y: matrixTranslation.y }, 'y').listen().name('TrY').onChange((value: number) => {
        //     Controls.onTransformChanged(matrixTranslation, matrixScale, matrixRotation, meshInstance);
        // });
        // transforms.add({ z: matrixTranslation.z }, 'z').listen().name('TrZ').onChange((value: number) => {
        //     Controls.onTransformChanged(matrixTranslation, matrixScale, matrixRotation, meshInstance);
        // });
        // transforms.add({ x: matrixRotation.x }, 'x').listen().name('RtX').onChange((value: number) => {
        //     Controls.onTransformChanged(matrixTranslation, matrixScale, matrixRotation, meshInstance);
        // });
        // transforms.add({ y: matrixRotation.y }, 'y').listen().name('RtY').onChange((value: number) => {
        //     Controls.onTransformChanged(matrixTranslation, matrixScale, matrixRotation, meshInstance);
        // });
        // transforms.add({ z: matrixRotation.z }, 'z').listen().name('RtZ').onChange((value: number) => {
        //     Controls.onTransformChanged(matrixTranslation, matrixScale, matrixRotation, meshInstance);
        // });
        // transforms.add({ x: matrixScale.x }, 'x').listen().name('ScX').onChange((value: number) => {
        //     Controls.onTransformChanged(matrixTranslation, matrixScale, matrixRotation, meshInstance);
        // });
        // transforms.add({ y: matrixScale.y }, 'y').listen().name('ScY').onChange((value: number) => {
        //     Controls.onTransformChanged(matrixTranslation, matrixScale, matrixRotation, meshInstance);
        // });
        // transforms.add({ z: matrixScale.z }, 'z').listen().name('ScZ').onChange((value: number) => {
        //     Controls.onTransformChanged(matrixTranslation, matrixScale, matrixRotation, meshInstance);
        // });
    ...
    static onMediumTypeChanged(materials: GUI, mediumType: GUI, value: MediumType, mat: Material, main: Main): GUI {
        mediumType.destroy();
        mediumType = materials.addFolder('Medium Type').close();
        ...
        if (value != MediumType.None)
        {
            ...
            mediumType.addColor({ rgb: { r: mediumColor.x, g: mediumColor.y, b: mediumColor.z} }, 'rgb').listen().name('Medium Color (Gamma Corrected)').onChange((value: { r: number, g: number, b: number}) => {
    ...
    // static onTransformChanged(matrixTranslation: Vec3, matrixScale: Vec3, matrixRotation: Vec3, meshInstance: MeshInstance): void {
    //     meshInstance.transform = Mat4.fromDecomposed(matrixTranslation, matrixScale, matrixRotation);
    // }
}