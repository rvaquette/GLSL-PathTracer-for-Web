...
class BufferView {
	...
	extras		:  any;
	data		:  ArrayBuffer;
	constructor(bufferViewBase: BufferViewBase, bufferData: ArrayBuffer, byteOffset: number = 0) {
		this.byteOffset = byteOffset + ((bufferViewBase.byteOffset	!== undefined) ? bufferViewBase.byteOffset: 0);
		this.byteLength =  bufferViewBase.byteLength;
		this.byteStride = (bufferViewBase.byteStride	!== undefined) ? bufferViewBase.byteStride	: 0;
		this.target		= (bufferViewBase.target		!== undefined) ? bufferViewBase.target		: null;
...
export class MeshPrimitive {
	attributesID			: { [key: string]: GLTFID } & {POSITION ?: GLTFID , NORMAL ?: GLTFID, TEXCOORD_0 ?: GLTFID};
	attributes				: { [key: string]: Accessor } & {POSITION ?: Accessor , NORMAL ?: Accessor, TEXCOORD_0 ?: Accessor};
	indicesID				:  GLTFID;
	indices					:  Accessor;
	primID					:  number;
	...
	constructor(primitiveBase: MeshPrimitiveBase, gltf: GLTF, currentLoader: GLTFLoader) {
		...
		this.materialID	= primitiveBase.material;
...
class Texture {
	...
	constructor(textureBase: TextureBase, currentLoader: GLTFLoader) {
		this.sampler	= textureBase.sampler;
		this.source   	= textureBase.source;
...
class Image {
	...
	mimeType	:  string;
	bufferView	:  number;
	image?		:  (ImageBitmap | ImageData | HTMLImageElement | null);
	rgba        : Uint8Array | null;
	extensions	:  any;
	extras		:  any;
	constructor(imageBase: ImageBase, currentLoader: GLTFLoader) {
		this.name		= (imageBase.name		!== undefined) ? imageBase.name				: null;
		this.mimeType	= (imageBase.mimeType	!== undefined) ? imageBase.mimeType			: null;
		this.bufferView	= (imageBase.bufferView	!== undefined) ? imageBase.bufferView		: null;
		this.extensions	= (imageBase.extensions	!== undefined) ? imageBase.extensions		: null;
		this.extras		= (imageBase.extras		!== undefined) ? imageBase.extras			: null;
	}
}
class Sampler {
	magFilter	:  SamplerMagnificationFilter;
...
export class GLTFLoader {
	...
	protected async postProcessAsync(glb: GLB = null):Promise<void> {
		if (glb == null) this.inferBufferViewTarget();
		// 挂载 BufferView
		if (this._glTF.bufferViews !== undefined) {
			for (let i = 0; i < this._glTF.bufferViews.length; i++) {
				const bufferData = glb == null ? 
					this.glTF.buffers[this._glTF.bufferViews[i].buffer]:
					glb.binChunks[this._glTF.bufferViews[i].buffer].arrayBuffer;
				const byteOffset = glb == null ? 
					0 :
					glb.binChunks[this._glTF.bufferViews[i].buffer].byteOffset;
				const bufferView = new BufferView(this._glTF.bufferViews[i], bufferData, byteOffset);
		...
		// Image
		if (this._glTF.images !== undefined && this.glTF.images.length === 0) {
			for (let i = 0; i < this._glTF.images.length; i++) {
				this.glTF.images.push(new Image(this._glTF.images[i], this));

				let imageBufferView = this.glTF.bufferViews[this._glTF.images[i].bufferView];
				let data = new Uint8Array(imageBufferView.data, imageBufferView.byteOffset, imageBufferView.byteLength);
        		if (typeof window !== "undefined") {
					this.glTF.images[i].image = await createImageBitmap(new Blob([data], { type: this._glTF.images[i].mimeType }));
				} else {
					this.glTF.images[i].rgba = data;
				}
			}
		}
		// 挂载 Mesh
	...
	public async loadGLTFAsync(uri: string): Promise<GLTF> {
		this.baseUri = this.getBaseUri(uri);
		try {
			const glTFBase : GLTFBase = await loadFile(uri).then((response) => {
				if (response.ok) {
		...
		const loadBuffer : Promise<boolean> = new Promise<boolean>(async (resolve) => {
			if (this._glTF.buffers) {
				...
				for (const bufferInfo of this._glTF.buffers) {
					try {
						bufferPromises.push(loadFile(this.baseUri + bufferInfo.uri).then((response) => {
							if (response.ok) {
		...
		const loadImage : Promise<boolean> = new Promise<boolean>(async (resolve) => {
			if (this._glTF.images) {
				const imagePromises : Promise<ImageBitmap | Uint8Array>[] = [];
				for (const imageInfo of this._glTF.images) {
					try {
						imagePromises.push(loadFile('/scenes/pathtracer/' + imageInfo.uri).then((response) => {
							if (response.ok) {
								return response.blob();
							}
							throw Error("LoadingError: Error occured in loading images.");
						}).then(async (imageBlob: Blob) => {
							if (typeof window !== "undefined") {
				...
				for (const [imageID, image] of imagePromises.entries()) {
					this.glTF.images[imageID] = new Image(this._glTF.images[imageID], this);
					const imageData = await image;
       				if (typeof window !== "undefined") {
						this.glTF.images[imageID].image = imageData as ImageBitmap;
					} else {
						this.glTF.images[imageID].rgba = imageData as Uint8Array;
					}
