...
// =============================================================================
//  Squelette de path tracer pour un matériau MaterialX (ex. glTF PBR "glass").
//
//  Hypothèses demandées :
//    * Les propriétés des MATERIAUX sont lues dans une texture sampler2D
//      (materialsTex), indexée par matID.
//    * Les propriétés des LUMIERES sont lues dans une texture sampler2D
//      (lightsTex), indexée par light index.
//
//  Ce fichier est un SQUELETTE : il fournit toute la "plomberie" (accès
//  textures, packing, State, points d'entrée du closure, intégrateur minimal).
//  Le coeur BSDF MaterialX (fonctions mx_*, IMPL_gltf_pbr_surfaceshader, etc.)
//  se colle tel quel dans la zone balisée __PROCEDURAL_MATERIAL_GENERATED__.
// =============================================================================
...
// Mode de rendu.
//  - Defini      : mode "gather" (option A) = reproduit EXACTEMENT la sequence
//                  de glslgenerator (REFLECTION par lumiere -> INDIRECT ->
//                  EMISSION -> TRANSMISSION) en UNE passe, sans rebonds
//                  (viewer biaise : indirect + transmission bases sur l'env).
//  - Non defini  : path tracer recursif (rebonds + NEE/MIS).
// #define OPT_MTLX_GATHER

// --- Layout de la texture matériaux -----------------------------------------
// Chaque matériau occupe MATERIALS_TEX_STRIDE vec4 contigus.
// Les 8 premiers vec4 sont ici réservés à un "bloc commun" (au choix), et le
// payload MaterialX commence à l'offset MAT_PAYLOAD.
// Adapte STRIDE au plus grand enregistrement matériau de ta scène.
#define MAT_PAYLOAD 8
#ifndef MATERIALS_TEX_STRIDE
#define MATERIALS_TEX_STRIDE 17   // 8 (commun) + 9 (payload glTF PBR)
#endif

// Nombre de vec4 par lumière dans lightsTex.
#define LIGHT_TEX_STRIDE 5

// Environnement (réutilisé du shader MaterialX d'origine ; sert de fond / miss).
...
#define u_envMatrix mat4(1.0)
#define u_envRadiance envMapTex
#define u_envIrradiance envMapCDFTex
...
// =============================================================================
//  1) PROPRIETES MATERIAUX : lecture depuis materialsTex (sampler2D)
// =============================================================================
//
//  Les uniforms scalaires du shader MaterialX d'origine deviennent des
//  variables globales, remplies par pt_LoadParams() à chaque intersection.
//  L'ORDRE de dépaquetage doit être identique au packing côté CPU.
//
//  Layout du payload glTF PBR (offset = matID*STRIDE + MAT_PAYLOAD) :
//    slot 0 : base_color.rgb                     | metallic
//    slot 1 : roughness | occlusion | transmission | specular
//    slot 2 : specular_color.rgb                 | ior
//    slot 3 : alpha | alpha_mode | alpha_cutoff  | iridescence
//    slot 4 : iridescence_ior | iridescence_thickness | sheen_color.rg
//    slot 5 : sheen_color.b | sheen_roughness | clearcoat | clearcoat_roughness
//    slot 6 : emissive.rgb                       | emissive_strength
//    slot 7 : thickness | attenuation_distance   | attenuation_color.rg
//    slot 8 : attenuation_color.b | anisotropy_strength | anisotropy_rotation | dispersion
// -----------------------------------------------------------------------------

// Variables globales matériau (mêmes noms que les uniforms d'origine).
vec3  base_color;
float metallic;
float roughness;
float occlusion;
...
{
    int base = matID * MATERIALS_TEX_STRIDE;

    vec4 b7 = texelFetch1D(materialsTex, base + 7);

    // Get MaterialType
    int materialType = int(b7.w + 0.5);

    if (materialType == 0) {
        vec4 b0 = texelFetch1D(materialsTex, base + 0);
        vec4 b2 = texelFetch1D(materialsTex, base + 2);
        ...
        vec4 s0 = texelFetch1D(materialsTex, base + MAT_PAYLOAD + 0);
        vec4 s1 = texelFetch1D(materialsTex, base + MAT_PAYLOAD + 1);
        vec4 s2 = texelFetch1D(materialsTex, base + MAT_PAYLOAD + 2);
        vec4 s3 = texelFetch1D(materialsTex, base + MAT_PAYLOAD + 3);
        vec4 s4 = texelFetch1D(materialsTex, base + MAT_PAYLOAD + 4);
        vec4 s5 = texelFetch1D(materialsTex, base + MAT_PAYLOAD + 5);
        vec4 s6 = texelFetch1D(materialsTex, base + MAT_PAYLOAD + 6);
        vec4 s7 = texelFetch1D(materialsTex, base + MAT_PAYLOAD + 7);
        vec4 s8 = texelFetch1D(materialsTex, base + MAT_PAYLOAD + 8);
        ...
        transmission          = s1.z;
        specular              = s1.w;
        specular_color        = s2.xyz;
        ior                   = s2.w;
        alpha                 = s3.x;
        alpha_mode            = int(s3.y + 0.5);
        alpha_cutoff          = s3.z;
        iridescence           = s3.w;
        iridescence_ior       = s4.x;
        iridescence_thickness = s4.y;
        sheen_color           = vec3(s4.z, s4.w, s5.x);
        sheen_roughness       = s5.y;
        clearcoat             = s5.z;
        clearcoat_roughness   = s5.w;
        emissive              = s6.xyz;
        emissive_strength     = s6.w;
        thickness             = s7.x;
        attenuation_distance  = s7.y;
        attenuation_color     = vec3(s7.z, s7.w, s8.x);
        anisotropy_strength   = s8.y;
        anisotropy_rotation   = s8.z;
        dispersion            = s8.w;
    }
}

// =============================================================================
//  2) PROPRIETES LUMIERES : lecture depuis lightsTex (sampler2D)
// =============================================================================
//
//  Layout par lumière (offset = i*LIGHT_TEX_STRIDE) :
//    slot 0 : position.xyz
//    slot 1 : emission.xyz
//    slot 2 : u.xyz          (bord 1 quad)
//    slot 3 : v.xyz          (bord 2 quad)
//    slot 4 : radius | area | type | (libre)
// -----------------------------------------------------------------------------
...
// =============================================================================
//  3) INTERSECTION DE LA SCENE  (à implémenter selon ta structure : BVH, SDF…)
// =============================================================================
//
//  ClosestHit doit remplir State (fhp, normal, tangent, bitangent, texCoord,
//  matID, eta) et renvoyer true si un rayon touche la géométrie.
//  AnyHit renvoie true dès qu'une occlusion est trouvée (pour les shadow rays).
//
//  >>> REMPLACE ces stubs par ton traversal (BVH + triangles, ou ray marching).
// -----------------------------------------------------------------------------
...
//
//  >>> COLLE ICI UNIQUEMENT la LIBRAIRIE MaterialX (depuis glass-pathtracergenerator.glsl) :
//        - structs BSDF / EDF / VDF / ClosureData + makeClosureData / CLOSURE_TYPE_*
//        - toutes les fonctions mx_* (fresnel, ggx, sheen, oren-nayar, dielectric…)
//        - NG_* et mx_rotate_vector3
//      NE COLLE PAS : IMPL_gltf_pbr_surfaceshader, mtlxEvalSurface, ni les
//      redeclarations de g_pt* / normalWorld / tangentWorld / struct surfaceshader
//      -> l'assemblage refactorise (entete + BSDF/EDF) est fourni ci-dessous.
//
//  Il consomme les variables globales matériau remplies par pt_LoadParams()
//  et les globales géométriques g_pt* remplies par les entry points ci-dessous.
//
//  NOTE : plus besoin de "Light loop" : l'éclairage direct est géré par
//  l'intégrateur (NEE + MIS) via lightsTex ; le closure n'evalue que le BSDF.
// =============================================================================

vec3 g_ptV, g_ptN, g_ptL, g_ptP, g_ptTangent, g_ptBitangent;
vec2 g_ptTexcoord;
int  g_ptClosureType;
...
{
    // Meme convention que EvalBackground() / shaders/common/envmap.glsl,
    // pour que l'environnement vu en mode gather corresponde au path tracer.
    float theta = acos(clamp(dir.y, -1.0, 1.0));
    float u = (M_PI + mx_atan(dir.z, dir.x)) * (0.5 * M_PI_INV);
    float v = theta * M_PI_INV;
    return vec2(u, v);
}
...
vec3 mx_latlong_map_lookup(vec3 dir, mat4 transform, float lod, sampler2D tex_sampler)
{
    ...
    // envMapRot : meme rotation horizontale que EvalBackground / envmap.glsl.
    vec2 uv = mx_latlong_projection(envDir) + vec2(envMapRot, 0.0);
...
// =============================================================================
//  pt_MtlxLayerStackResponse : empilement de closures MaterialX (le "stack" de
//  IMPL_gltf_pbr_surfaceshader), renvoie clearcoat_layer.response pour un
//  closureType donne. Factorise car appele plusieurs fois par le gather
//  (REFLECTION x lumieres, INDIRECT, TRANSMISSION) ET par mtlxEvalSurface.
//    (A) entete (hors BSDF) recalculee ici ; (B) empilement de closures.
// =============================================================================
vec3 pt_MtlxLayerStackResponse(int closureType, vec3 L, vec3 V, vec3 N, vec3 P, vec3 T, float occlusion)
{
    // (A) ENTETE : parametres derives (hors BSDF)
    vec2 clearcoat_roughness_uv;
    mx_roughness_anisotropy(clearcoat_roughness, 0.0, clearcoat_roughness_uv);

    float sheen_intensity;
    NG_maxcomponent_color3(sheen_color, sheen_intensity);
    vec3  sheen_color_normalized = sheen_color / sheen_intensity;
    float sheen_roughness_sq     = sheen_roughness * sheen_roughness;

    float alpha_roughness = roughness * roughness;
    float strength_2      = anisotropy_strength * anisotropy_strength;
    float clamped_at      = clamp(mix(alpha_roughness, 1.0, strength_2), 1e-5, 1.0);
    float clamped_ab      = clamp(alpha_roughness, 1e-5, 1.0);
    vec2  roughness_uv    = vec2(clamped_at, clamped_ab);

    float rad_2_deg = anisotropy_rotation * -57.295780;
    vec3  rotate_tangent = vec3(0.0);
    mx_rotate_vector3(T, rad_2_deg, N, rotate_tangent);
    vec3 selected_tangent = (abs(anisotropy_rotation) > 0.0) ? normalize(rotate_tangent) : T;

    float metal_fg_weight      = iridescence;
    float metal_bg_weight      = 1.0 - iridescence;
    float dielectric_fg_weight = iridescence;
    float dielectric_bg_weight = 1.0 - iridescence;

    float ior_div                = (1.0 - ior) / (1.0 + ior);
    float dielectric_f0_from_ior = ior_div * ior_div;
    vec3  dielectric_f0  = min(specular_color * dielectric_f0_from_ior, 1.0) * specular;
    vec3  dielectric_f90 = vec3(1.0) * specular;

    float transmission_fg_weight = transmission;
    float transmission_bg_weight = 1.0 - transmission;
    float base_mix_inv           = 1.0 - metallic;

    ClosureData cd = makeClosureData(closureType, L, V, N, P, occlusion);

    // (B) BSDF : empilement de closures (identique a IMPL_gltf_pbr_surfaceshader)
    BSDF clearcoat_bsdf = BSDF(vec3(0.0), vec3(1.0));
    mx_dielectric_bsdf(cd, clearcoat, vec3(1.0), 1.5, clearcoat_roughness_uv, false, 0.0, 1.5, N, T, 0, 0, clearcoat_bsdf);

    BSDF sheen_bsdf = BSDF(vec3(0.0), vec3(1.0));
    mx_sheen_bsdf(cd, sheen_intensity, sheen_color_normalized, sheen_roughness_sq, N, 0, sheen_bsdf);

    BSDF tf_metal = BSDF(vec3(0.0), vec3(1.0));
    mx_generalized_schlick_bsdf(cd, metal_fg_weight, base_color, vec3(1.0), vec3(1.0), 5.0, roughness_uv, false, iridescence_thickness, iridescence_ior, N, selected_tangent, 0, 0, tf_metal);
    BSDF metal = BSDF(vec3(0.0), vec3(1.0));
    mx_generalized_schlick_bsdf(cd, metal_bg_weight, base_color, vec3(1.0), vec3(1.0), 5.0, roughness_uv, false, 0.0, 1.5, N, selected_tangent, 0, 0, metal);
    BSDF metal_add = BSDF(vec3(0.0), vec3(1.0));
    mx_add_bsdf(cd, tf_metal, metal, metal_add);
    BSDF base_mix_fg = BSDF(vec3(0.0), vec3(1.0));
    mx_multiply_bsdf_float(cd, metal_add, metallic, base_mix_fg);

    BSDF tf_refl = BSDF(vec3(0.0), vec3(1.0));
    mx_generalized_schlick_bsdf(cd, dielectric_fg_weight, dielectric_f0, vec3(1.0), dielectric_f90, 5.0, roughness_uv, false, iridescence_thickness, iridescence_ior, N, selected_tangent, 0, 0, tf_refl);
    BSDF refl = BSDF(vec3(0.0), vec3(1.0));
    mx_generalized_schlick_bsdf(cd, dielectric_bg_weight, dielectric_f0, vec3(1.0), dielectric_f90, 5.0, roughness_uv, false, 0.0, 1.5, N, selected_tangent, 0, 0, refl);
    BSDF refl_add = BSDF(vec3(0.0), vec3(1.0));
    mx_add_bsdf(cd, tf_refl, refl, refl_add);

    BSDF trans = BSDF(vec3(0.0), vec3(1.0));
    mx_dielectric_bsdf(cd, transmission_fg_weight, base_color, ior, roughness_uv, false, 0.0, 1.5, N, selected_tangent, 0, 1, trans);
    BSDF diffuse = BSDF(vec3(0.0), vec3(1.0));
    mx_oren_nayar_diffuse_bsdf(cd, transmission_bg_weight, base_color, 0.0, N, false, diffuse);
    BSDF trans_add = BSDF(vec3(0.0), vec3(1.0));
    mx_add_bsdf(cd, trans, diffuse, trans_add);

    BSDF dielectric = BSDF(vec3(0.0), vec3(1.0));
    mx_layer_bsdf(cd, refl_add, trans_add, dielectric);
    BSDF base_mix_bg = BSDF(vec3(0.0), vec3(1.0));
    mx_multiply_bsdf_float(cd, dielectric, base_mix_inv, base_mix_bg);

    BSDF base_mix = BSDF(vec3(0.0), vec3(1.0));
    mx_add_bsdf(cd, base_mix_fg, base_mix_bg, base_mix);

    BSDF sheen_layer = BSDF(vec3(0.0), vec3(1.0));
    mx_layer_bsdf(cd, sheen_bsdf, base_mix, sheen_layer);
    BSDF clearcoat_layer = BSDF(vec3(0.0), vec3(1.0));
    mx_layer_bsdf(cd, clearcoat_bsdf, sheen_layer, clearcoat_layer);

    return clearcoat_layer.response;
}

// =============================================================================
//  mtlxEvalSurface : dispatch d'UN closure (mode path tracer). Delegue le BSDF
//  a pt_MtlxLayerStackResponse et gere l'emission via l'EDF.
// =============================================================================
surfaceshader mtlxEvalSurface(State state)
{
    // Geometrie : varyings d'origine remplaces par les globales g_pt*
    vec3 normal  = normalize(g_ptN);
    vec3 tangent = normalize(g_ptTangent);

    // Opacite (convention MaterialX gltf : 0=OPAQUE, 1=MASK, 2=BLEND)
    float opacity_mask_cutoff = (alpha >= alpha_cutoff) ? 1.0 : 0.0;
    float opacity_mask        = (alpha_mode == 1) ? opacity_mask_cutoff : alpha;
    float opacity             = (alpha_mode == 0) ? 1.0 : opacity_mask;

    surfaceshader result = surfaceshader(vec3(0.0), vec3(0.0));

    if (g_ptClosureType == CLOSURE_TYPE_EMISSION)
    {
        // (C) EDF : emission
        ClosureData cd = makeClosureData(g_ptClosureType, g_ptL, g_ptV, normal, g_ptP, 1.0);
        EDF emission = EDF(0.0);
        mx_uniform_edf(cd, emissive * emissive_strength, emission);
        result.color = emission;
    }
    else
    {
        // (B) BSDF : REFLECTION / TRANSMISSION / INDIRECT
        result.color = pt_MtlxLayerStackResponse(g_ptClosureType, g_ptL, g_ptV, normal, g_ptP, tangent, 1.0);
    }
...
// =============================================================================
//  4) POINTS D'ENTREE DU CLOSURE (remplacent le main() du shader rastérisé)
// =============================================================================
//
//  EvalMtlxClosure   : évalue f(V,N,L) et le pdf pour une direction donnée (MIS).
//  SampleMtlxClosure : échantillonne une nouvelle direction L + renvoie f et pdf.
//
//  Dispatch par CONTEXTE de closure MaterialX :
//   - REFLECTION   (dot(N,L) > 0) : reponse complete du stack via
//                   pt_MtlxLayerStackResponse(CLOSURE_TYPE_REFLECTION, ...) ;
//                   c'est la vraie BRDF (metal + dielectrique + sheen +
//                   clearcoat + diffus), cosinus deja inclus.
//   - TRANSMISSION (dot(N,L) < 0) : BTDF dielectrique microfacette analytique.
//                   Le closure CLOSURE_TYPE_TRANSMISSION de MaterialX est base
//                   sur l'environnement (mx_surface_transmission) et ignore L,
//                   donc inutilisable pour un vrai rayon refracte -> on evalue
//                   une BTDF (Walter et al.) coherente avec les mx_ggx_*.
//   - INDIRECT : l'eclairage environnement est capture par les rebonds tracés
//                (le rayon qui rate la geometrie echantillonne EvalBackground),
//                on n'appelle donc PAS le closure INDIRECT ici (pas de double
//                comptage).
// -----------------------------------------------------------------------------
...
// Rugosite microfacette (meme convention que le stack : alpha = roughness^2).
vec2 pt_MtlxAlpha()
{
    float a = clamp(roughness * roughness, 1e-4, 1.0);
    return vec2(a, a);
}
...
{
    if (L.z >= 0.0) return 0.0;
    // Demi-vecteur de refraction : eta multiplie V (coherent avec refract(-V,H,eta)).
...
vec3 pt_TransBtdf(vec3 V, vec3 L, vec2 alpha, float eta, float F, vec3 tint)
{
    ...
    denom *= denom;
    float jacobian = abs(LdotH) / max(denom, 1e-7);
    // f * |L.z| : le |L.z| se simplifie avec le denominateur BTDF.
                     ...
                     out float pdf, out int flags)
{
    ...
    // Refraction : eta multiplie Vl (coherent avec refract(-Vl,H,eta)).
    ...
    // Poids des lobes -> probabilites (identiques cote SampleMtlxClosure).
    float wSpec  = mix(F, 1.0, metallic);
    float wTrans = (1.0 - metallic) * transmission * (1.0 - F);
    float wDiff  = (1.0 - metallic) * (1.0 - transmission);
    ...
    if (reflectSide)
    {
        // (A) REFLECTION : reponse complete du stack MaterialX (BRDF * cos).
        vec3 f = pt_MtlxLayerStackResponse(CLOSURE_TYPE_REFLECTION, L, V, N,
                                           state.fhp, state.tangent, 1.0);
        // pdf : melange lobe speculaire (VNDF) + lobe diffus (cosine).
        pdf = pSpec * pt_ReflPdf(Vl, Ll, alpha) + pDiff * max(Ll.z, 0.0) * INV_PI;
        return f;
    }
    else
    {
        // (B) TRANSMISSION : BTDF dielectrique analytique (vrai rayon refracte).
        vec3 tint = sqrt(max(base_color, 0.0)); // teinte appliquee aux 2 interfaces
                       ...
                       out vec3 L, out float pdf, out int flags)
{
    ...
    // Demi-vecteur GGX (VNDF) partage par la reflexion et la refraction.
    ...
    vec3 Ll;
    float u = rand();
    if (u < pSpec)                       // reflexion speculaire (REFLECTION)
    {
        Ll = reflect(-Vl, H);
    }
    else if (u < pSpec + pTrans)         // refraction (TRANSMISSION)
    {
        Ll = refract(-Vl, H, eta);
        if (dot(Ll, Ll) < 1e-8)          // reflexion totale interne -> reflexion
            Ll = reflect(-Vl, H);
    }
    else                                 // diffus (hemisphere +, REFLECTION)
    {
    ...
    // Delegue a Eval : f et pdf coherents (meme melange de lobes -> non biaise).
    return EvalMtlxClosure(matID, state, V, N, L, pdf, flags);
}

// =============================================================================
//  5) ECLAIRAGE DIRECT (NEE + MIS) via lightsTex
// =============================================================================
...
// =============================================================================
//  5bis) MODE GATHER (option A) : rejoue la sequence de glslgenerator
//        REFLECTION (par lumiere) -> INDIRECT -> EMISSION -> TRANSMISSION,
//        en un seul point de shading (pas de recursion).
// =============================================================================

// Convertit une lumiere surfacique (lightsTex) en "directionnelle equivalente"
// (direction + irradiance), comme le lightshader de glslgenerator. Estimation
// a un echantillon pour les lumieres d'aire.
vec3 pt_LightToDirectional(Light l, vec3 P, out vec3 dir, out float dist)
{
    if (int(l.type) == DISTANT_LIGHT)
    {
        dir  = normalize(l.position);   // direction stockee dans position
        dist = INF;
        return l.emission;
    }
    vec3 sp;
    ...
    // Irradiance ~ Le * (aire * cosLumiere) / dist^2 (le cosinus surface est
    // pris en compte a l'interieur des closures, comme dans glslgenerator).
...
vec3 mtlxShadeGather(State state, Ray r)
{
    ...
    // Opacite de surface (convention MaterialX gltf : 0=OPAQUE,1=MASK,2=BLEND)
    float opacity_mask_cutoff = (alpha >= alpha_cutoff) ? 1.0 : 0.0;
    float opacity_mask        = (alpha_mode == 1) ? opacity_mask_cutoff : alpha;
    float surfaceOpacity      = (alpha_mode == 0) ? 1.0 : opacity_mask;
    ...
    for (int i = 0; i < numOfLights; i++)
    {
        ...
        // Occlusion (shadow ray). glslgenerator laisse occlusion=1 ; on teste
        // ici la visibilite pour un rendu correct.
        float occ = 1.0;
    ...
    // (2) INDIRECT : environnement (mx_environment_radiance/irradiance)
    color += pt_MtlxLayerStackResponse(CLOSURE_TYPE_INDIRECT, vec3(0.0), V, N, P, T, 1.0);

    // (3) EMISSION
    {
        ClosureData cd = makeClosureData(CLOSURE_TYPE_EMISSION, vec3(0.0), V, N, P, 1.0);
        EDF emission = EDF(0.0);
        mx_uniform_edf(cd, emissive * emissive_strength, emission);
        color += emission;
    }

    // (4) TRANSMISSION : refraction basee sur l'environnement (direction de vue)
...
// =============================================================================
//  6) INTEGRATEUR (path tracing avec rebonds)
// =============================================================================
vec3 PathTrace(Ray r)
{
    ...
    for (int depth = 0; depth < maxDepth; depth++)
    {
        ...
        // Charge les propriétés matériau depuis la texture pour ce hit.
        pt_LoadParams(state.matID);

        // Ratio d'indices (ni/nt) pour la refraction : 1/ior en entrant dans le
        // milieu (rayon oppose a la normale geometrique), ior en sortant.
        state.eta = dot(r.direction, state.normal) < 0.0 ? (1.0 / max(ior, 1.001)) : max(ior, 1.001);

        // Emission de surface.
        radiance += throughput * emissive * emissive_strength;

        // Eclairage direct (NEE).
        radiance += throughput * DirectLight(r, state);

        // Rebond indirect : échantillonne le BSDF.
        vec3 L;
        float pdf;
        int flags;
        vec3 f = SampleMtlxClosure(state.matID, state, -r.direction,
                                   state.ffnormal, L, pdf, flags);
        if (pdf <= 0.0)
            break;
...
// =============================================================================
//  7) GENERATION DU RAYON PRIMAIRE + main()
// =============================================================================
...
void main()
{
...
#ifdef OPT_MTLX_GATHER
    // Mode gather (option A) : un seul point de shading, sequence glslgenerator.
    vec3 pixelColor;
    State state;
    state.depth = 0;
    state.eta = 1.0;
    ...
    // Sortie linéaire HDR : l'accumulation et le tone-mapping/gamma se font
    // dans une passe séparée (ne pas appliquer mx_srgb_encode ici).
