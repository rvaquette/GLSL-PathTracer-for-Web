...
import { ShaderInclude } from "./shaderIncludes.js";
import { Denoiser } from "../external/denoiser/denoiser.js";
export class Renderer {
    static maxBufferTextureWidth = 4096;
    gl;
    _scene;
    shadersDirectory;
    BVHTex = null;
    vertexIndicesTex = null;
    verticesTex = null;
    normalsTex = null;
    materialsTex = null;
    transformsTex = null;
    lightsTex = null;
    textureMapsArrayTex = null;
    envMapTex = null;
    envMapCDFTex = null;
    pathTraceTextureLowRes = null;
    pathTraceTexture = null;
    accumTexture = null;
    tileOutputTexture = [null, null];
    pathTraceFBO = null;
    pathTraceFBOLowRes = null;
    accumFBO = null;
    outputFBO = null;
    pathTraceShader = null;
    pathTraceShaderLowRes = null;
    outputShader = null;
    tonemapShader = null;
    programs = [];
    quad;
    ...
    backendReady = false;
    denoiser = null;
    denoiserExecutedOneTime = false;
    denoised = false;
    denoiserFBO = null;
    denoiserTexture = null;
    denoiserInputFramePtr = null;
    denoiserCanvas = null;
    _sampleCounter = 1;
    currentBuffer = 0;
    frameCounter = 1;
    _renderSize = new Vec2(0, 0);
    windowSize = new Vec2(0, 0);
    tileWidth = 0;
    tileHeight = 0;
    invNumTiles = new Vec2(0, 0);
    numTiles = new Vec2(0, 0);
    tile = new Vec2(0, 0);
    get scene() {
        return this._scene;
    }
    ...
    createBufferTexture(data, format) {
        ...
        return this.createTexture(format === 4 ? this.gl.raw.RGBA32F : this.gl.raw.RGB32F, width, height, format === 4 ? this.gl.raw.RGBA : this.gl.raw.RGB, this.gl.raw.FLOAT, data);
    }
    createBufferTextureInt(data, format) {
        const totalElements = data.length / format;
        const width = Math.min(Renderer.maxBufferTextureWidth, totalElements);
        const height = Math.ceil(totalElements / width);
        if (width * height * format !== data.length) {
        ...
        return this.createTexture(format === 4 ? this.gl.raw.RGBA32I : this.gl.raw.RGB32I, width, height, format === 4 ? this.gl.raw.RGBA_INTEGER : this.gl.raw.RGB_INTEGER, this.gl.raw.INT, data);
    }
    createBufferTextureUint(data, format) {
        const totalElements = data.length / format;
        const width = this.scene.renderOptions.texArrayWidth;
        const height = Math.ceil(totalElements / width);
        return this.createTexture(format === 4 ? this.gl.raw.RGBA8UI : this.gl.raw.RGB8UI, width, height, format === 4 ? this.gl.raw.RGBA_INTEGER : this.gl.raw.RGB_INTEGER, this.gl.raw.UNSIGNED_BYTE, data);
    }
    initGPUDataBuffers() {
        const gl = this.gl;
        ...
        gl.activeTexture(gl.raw.TEXTURE1);
        gl.bindTexture(gl.raw.TEXTURE_2D, this.BVHTex);
        gl.activeTexture(gl.raw.TEXTURE2);
        gl.bindTexture(gl.raw.TEXTURE_2D, this.vertexIndicesTex);
        gl.activeTexture(gl.raw.TEXTURE3);
        gl.bindTexture(gl.raw.TEXTURE_2D, this.verticesTex);
        gl.activeTexture(gl.raw.TEXTURE4);
        gl.bindTexture(gl.raw.TEXTURE_2D, this.normalsTex);
        gl.activeTexture(gl.raw.TEXTURE5);
        gl.bindTexture(gl.raw.TEXTURE_2D, this.materialsTex);
        gl.activeTexture(gl.raw.TEXTURE6);
        gl.bindTexture(gl.raw.TEXTURE_2D, this.transformsTex);
        gl.activeTexture(gl.raw.TEXTURE7);
        gl.bindTexture(gl.raw.TEXTURE_2D, this.lightsTex);
        gl.activeTexture(gl.raw.TEXTURE8);
        gl.bindTexture(gl.raw.TEXTURE_2D, this.textureMapsArrayTex);
        gl.activeTexture(gl.raw.TEXTURE9);
        gl.bindTexture(gl.raw.TEXTURE_2D, this.envMapTex);
        gl.activeTexture(gl.raw.TEXTURE10);
        gl.bindTexture(gl.raw.TEXTURE_2D, this.envMapCDFTex);
    }
    dispose() {
        const gl = this.gl;
    ...
    async initFBOsAsync() {
        ...
        this.pathTraceTextureLowRes = this.createTexture(gl.raw.RGBA32F, Math.floor(this.windowSize.x * this.pixelRatio), Math.floor(this.windowSize.y * this.pixelRatio), gl.raw.RGBA, gl.raw.FLOAT, null);
        ...
        await new Promise((resolve) => {
            this.denoiser.onBackendReady(() => {
                ...
                this.denoiser.onExecute((frameOutputPtr) => {
                    ...
                    if (!this.denoiserExecutedOneTime)
                        this.denoiserExecutedOneTime = true;
                }, "float32");
    ...
    async initShadersAsync() {
        const gl = this.gl;
        const scene = this.scene;
        const [vertexShaderSrc, pathTraceShaderSrc, pathTraceShaderLowResSrc, outputShaderSrc, tonemapShaderSrc] = await Promise.all([
        ...
        const insertDefines = (obj, defines) => {
    ...
    exportTextureToImage(texture, width, height, filename) {
        if (!texture) {
            return;
        }
        const gl = this.gl;
    ...
    async updateAsync(_secondsElapsed, _secondsElapsedDelta) {
        const gl = this.gl;
        const scene = this.scene;
