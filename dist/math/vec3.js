export class Vec3 {
    x;
    y;
    z;
    constructor(x = 0, y = 0, z = 0) {
        this.x = x;
        this.y = y;
        this.z = z;
    }
    toArray() {
        return [this.x, this.y, this.z];
    }
    clone() {
        return new Vec3(this.x, this.y, this.z);
    }
    static fromVec4(b) {
        return new Vec3(b.x, b.y, b.z);
    }
    multiply(b) {
        return new Vec3(this.x * b.x, this.y * b.y, this.z * b.z);
    }
    static add(a, b) {
        return a.add(b);
    }
    add(b) {
        return new Vec3(this.x + b.x, this.y + b.y, this.z + b.z);
    }
    static subtract(a, b) {
        return a.subtract(b);
    }
    subtract(b) {
        return new Vec3(this.x - b.x, this.y - b.y, this.z - b.z);
    }
    static scale(a, b) {
        return a.scale(b);
    }
    scale(b) {
        return new Vec3(this.x * b, this.y * b, this.z * b);
    }
    get(i) {
        if (i === 0)
            return this.x;
        if (i === 1)
            return this.y;
        return this.z;
    }
    set(i, value) {
        if (i === 0)
            this.x = value;
        else if (i === 1)
            this.y = value;
        else
            this.z = value;
    }
    static log(a) {
        return new Vec3(Math.log(a.x), Math.log(a.y), Math.log(a.z));
    }
    static negate(a) {
        return new Vec3(-a.x, -a.y, -a.z);
    }
    static min(a, b) {
        return new Vec3(Math.min(a.x, b.x), Math.min(a.y, b.y), Math.min(a.z, b.z));
    }
    static max(a, b) {
        return new Vec3(Math.max(a.x, b.x), Math.max(a.y, b.y), Math.max(a.z, b.z));
    }
    static cross(a, b) {
        return new Vec3(a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z, a.x * b.y - a.y * b.x);
    }
    static pow(a, exp) {
        return new Vec3(Math.pow(a.x, exp), Math.pow(a.y, exp), Math.pow(a.z, exp));
    }
    static Length(a) {
        return Math.sqrt(a.x * a.x + a.y * a.y + a.z * a.z);
    }
    static dot(a, b) {
        return a.x * b.x + a.y * b.y + a.z * b.z;
    }
    static distance(a, b) {
        return Vec3.Length(a.subtract(b));
    }
    static clamp(a, min, max) {
        return new Vec3(Math.max(min.x, Math.min(a.x, max.x)), Math.max(min.y, Math.min(a.y, max.y)), Math.max(min.z, Math.min(a.z, max.z)));
    }
    static normalize(a) {
        const l = Vec3.Length(a);
        return new Vec3(a.x / l, a.y / l, a.z / l);
    }
}
//# sourceMappingURL=vec3.js.map