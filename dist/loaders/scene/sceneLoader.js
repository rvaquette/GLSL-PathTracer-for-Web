...
function getInt3Array(buffer, offset1, offset2) {
    let tmp1 = buffer.subarray(offset1 * 4, offset2 * 4);
    let tmp2 = [];
    for (let i = 0; i < tmp1.length; i += 4) {
        tmp2.push(new Vec3(tmp1[i], tmp1[i + 1], tmp1[i + 2]));
...
export function getImageDataCore(image, width, height, flip = false) {
...
export function getImageData(image, width, height, flip = false) {
